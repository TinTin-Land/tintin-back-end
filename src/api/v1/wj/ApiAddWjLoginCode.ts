...
import {ReqAddWjLoginCode, ResAddWjLoginCode} from "../../../shared/protocols/v1/wj/PtlAddWjLoginCode";
...
export default async function (call: ApiCall<ReqAddWjLoginCode, ResAddWjLoginCode>) {
    ...
    if (third_party_user == undefined ){
        ...
        const user_wj_login_code_before = await getRepository(User_wj_login_code).createQueryBuilder("user_wj_login_code")
            .where("user_wj_login_code.user_email = :user_email", { user_email })
            .getOne();

        if (user_wj_login_code_before?.user_email == user_email){
            user_wj_login_code_before.login_code = response.data.data.code;
            await getRepository(User_wj_login_code).save(user_wj_login_code_before);
            await call.succ({
                time,
            ...
            await getRepository(User_wj_login_code).insert(user_wj_login_code);
            await call.succ({
                time,
                code:response.data.data.code
            });
